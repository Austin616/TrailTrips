...
export function Navigation(){const pathname=usePathname();const {user}=useAuth();return <header className="site-header"><Link href="/" className="brand"><span className="brand-icon"><Mountain size={25} strokeWidth={1.8}/></span>trailtrips<span className="brand-dot">.</span></Link><nav aria-label="Main navigation"><Link className={pathname==='/explore'?'active':''} href="/explore">Explore</Link><Link className={pathname.startsWith('/trips')?'active':''} href="/trips">My trips</Link></nav><div className="nav-action">{user&&<CreateTripDialog label="Plan a trip"/>}<AccountMenu/></div></header>}
